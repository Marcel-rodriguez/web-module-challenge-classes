...
class Person {
  ...
  eat(food){
    this.stomach.length < 10 ? this.stomach = [...this.stomach, food] : this.stomach
  }
  poop(){
    this.stomach = []
  }
...
class Car {
  ...
  drive(distance){
    let totalDrivable = this.tank * this.milesPerGallon
    if(distance > totalDrivable){
      this.odometer += totalDrivable
      this.tank = 0
